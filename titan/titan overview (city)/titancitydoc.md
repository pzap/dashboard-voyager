...
all documentation about TNPS cardinality overview dashboard ```updated Nov 7th 2022```
...
![Titan Overview - subs overview](https://github.com/pzap/dashboard-voyager/blob/main/titan/titan%20overview%20(city)/Titan%20Overview%20-%20Link%20to%20Network.png)
1. data sources, 
2. columns, 
...
|--------------------|--------------------------|---------------------------|------------------------|------|
| All City | DSN_MMA_CDH_IMPALA_MMAID | sbx_titan.cei_city | ```select * from sbx_titan.cei_city```<br />select * from sbx_titan.cei_city where week in (with c as (select distinct(concat(month,week)), month, week from sbx_titan.cei_city order by (concat(month,week)) desc limit 6) select week from c) and month in (with c as (select distinct(concat(month,week)), month, week from sbx_titan.cei_city order by (concat(month,week)) desc limit 6) select month from c)<br /> ```updated in oct 24th 2022```  | sql |
...
| City List | - | TITAN DASHBOARD URL | let<br />Source = Excel.Workbook(Web.Contents<br />("https://xquisuite-my.sharepoint.com/personal/<br />permana_xquisite_ai/Documents/<br />TITAN%20DASHBOARD%20URL.xlsx"), null, true),<br />Table2_Table = Source{[Item="Table2",Kind="Table"]}[Data],<br />#"Changed Type" = Table.TransformColumnTypes(Table2_Table,{{"City", type text}, {"Province", type text}, {"Web URL", type text}}),<br />#"Uppercased Text" = Table.TransformColumns(#"Changed Type",{{"City", Text.Upper, type text}})<br />in<br />#"Uppercased Text"<br /> ```added in Nov 3rd 2022```| mcode (one drive)|
...
|	Subs Total N	|	```Subs Total N = SWITCH('by Parameter'[Chose Value Parameter],```<br />```1,SUM('All City'[data_availability_n_subs]),```<br />```2,SUM('All City'[web_browsing_n_subs]),```<br />```3,SUM('All City'[streaming_n_subs]),```<br />```4,SUM('All City'[social_media_n_subs]),```<br />```5,SUM('All City'[gaming_n_subs]))```<br />Subs Total N = SWITCH('by Parameter'[Chose Value Parameter],<br />1,CALCULATE(SUM('All City'[data_availability_n_subs]),'All City'[week]=max('All City'[week])),<br />2,CALCULATE(SUM('All City'[web_browsing_n_subs]),'All City'[week]=max('All City'[week])),<br />3,CALCULATE(SUM('All City'[streaming_n_subs]),'All City'[week]=max('All City'[week])),<br />4,CALCULATE(SUM('All City'[social_media_n_subs]),'All City'[week]=max('All City'[week])),<br />5,CALCULATE(SUM('All City'[gaming_n_subs]),'All City'[week]=max('All City'[week])))<br />```Updated in Oct 20th 2022```|
|	Subs Detractor N	|	```SWITCH('by Parameter'[Chose Value Parameter],```<br />```1,SUM('All City'[data_availability_detractor_n]),```<br />```2,SUM('All City'[web_browsing_detractor_n]),```<br />```3,SUM('All City'[streaming_detractor_n]),```<br />```4,SUM('All City'[social_media_detractor_n]),```<br />```5,SUM('All City'[gaming_detractor_n]))```<br />SWITCH('by Parameter'[Chose Value Parameter],<br />1,SUM('All City Maxy'[data_availability_detractor_n]),<br />2,SUM('All City Maxy'[web_browsing_detractor_n]),<br />3,SUM('All City Maxy'[streaming_detractor_n]),<br />4,SUM('All City Maxy'[social_media_detractor_n]),<br />5,SUM('All City Maxy'[gaming_detractor_n]))<br /> ```Updated in Oct 20th 2022```	|
|	Subs Passive N	|	```SWITCH('by Parameter'[Chose Value Parameter],```<br />```1,SUM('All City'[data_availability_passive_n]),```<br />```2,SUM('All City'[web_browsing_passive_n]),```<br />```3,SUM('All City'[streaming_passive_n]),```<br />```4,SUM('All City'[social_media_passive_n]),```<br />```5,SUM('All City'[gaming_passive_n]))```<br />	SWITCH('by Parameter'[Chose Value Parameter],<br />1,SUM('All City Maxy'[data_availability_passive_n]),<br />2,SUM('All City Maxy'[web_browsing_passive_n]),<br />3,SUM('All City Maxy'[streaming_passive_n]),<br />4,SUM('All City Maxy'[social_media_passive_n]),<br />5,SUM('All City Maxy'[gaming_passive_n]))<br />```Updated in Oct 20th 2022```|
|	Subs Promotor N	|	```SWITCH('by Parameter'[Chose Value Parameter],<br />1,SUM('All City'[data_availability_promotor_n]),<br />2,SUM('All City'[web_browsing_promotor_n]),<br />3,SUM('All City'[streaming_promotor_n]),<br />4,SUM('All City'[social_media_promotor_n]),<br />5,SUM('All City'[gaming_promotor_n]))```<br />SWITCH('by Parameter'[Chose Value Parameter],<br />1,SUM('All City Maxy'[data_availability_promotor_n]),<br />2,SUM('All City Maxy'[web_browsing_promotor_n]),<br />3,SUM('All City Maxy'[streaming_promotor_n]),<br />4,SUM('All City Maxy'[social_media_promotor_n]),<br />5,SUM('All City Maxy'[gaming_promotor_n]))<br />```Updated in Oct 20th 2022```	|
